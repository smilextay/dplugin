window.dpPlugins = window.dpPlugins || function (t) {
    ...
    class HlsEvents {
        ...
        switchVideo() {
            ...
            const videoHTML =
                '<video class="dplayer-video" webkit-playsinline playsinline crossorigin="anonymous" preload="auto" src="' +
                this.player.quality.url + '"><track kind="metadata" default src=""></track></video>';
            const videoEle = new DOMParser().parseFromString(videoHTML, 'text/html').body.firstChild;
            this.player.template.videoWrap.insertBefore(videoEle, this.player.template.videoWrap.getElementsByTagName(
                'div')[0]);
            ...
            this.player.video.oncanplaythrough = () => {
                ...
                this.player.video.currentTime = Math.max(this.player.prevVideo.currentTime, currentTime +
                    (Date.now() - now) / 1000);
                ...
                this.player.video.onplaying = () => {
                    ...
                    this.player.video.currentTime = Math.max(this.player.prevVideo.currentTime,
                        currentTime + (Date.now() - now) / 1000);
                    ...
                    while (this.player.template.videoWrap.querySelectorAll('video').length > 1) {
                        this.player.template.videoWrap.removeChild(this.player.template.videoWrap.getElementsByTagName(
                            'video')[1]);
                    }
        ...
        onEvents() {
            ...
            this.hls.once(Hls.Events.ERROR, (event, data) => {
                if (this.isUrlExpires(data ? .frag ? .relurl || data ? .response ? .url || this.hls
                        .url)) {
    ...
    class ImageEnhancer {
        constructor(player, obj) {
            ...
            this.player.template.imageEnhancer = obj.append(obj.query('.dplayer-setting-origin-panel', this
                    .player.template.settingBox),
            ...
            this.player.template.imageEnhancer.addEventListener('click', () => {
                this.imageenhancer = this.player.template.imageEnhancerToggle.checked = !this.player
                    .template.imageEnhancerToggle.checked;
    ...
    class SoundEnhancer {
        constructor(player, obj) {
            ...
            this.player.user.init();
            /*** SoundEnhancer ***/
            this.player.template.soundEnhancer = obj.append(obj.query('.dplayer-setting-origin-panel', this
                    .player.template.settingBox),
            ...
            this.player.template.soundEnhancer.addEventListener('click', () => {
                let checked = this.player.template.soundEnhancerToggle.checked = !this.player.template
                    .soundEnhancerToggle.checked;
                this.player.user.set("soundenhancer", Number(checked));
                this.switchJoysound(checked);
            });
            /*** VolumeEnhancer ***/
            this.player.template.gainBox = obj.prepend(obj.query('.dplayer-setting-origin-panel', this.player
                    .template.settingBox),
            ...
            this.player.template.gainBarWrap = this.player.template.gainBox.querySelector(
                '.dplayer-gain-bar-wrap');
            this.player.bar.elements.gain = this.player.template.gainBox.querySelector(
                '.dplayer-gain-bar-inner');
            const gainMove = (event) => {
                const e = event || window.event;
                let percentage = ((e.clientX || e.changedTouches[0].clientX) - this.getElementViewLeft(
                    this.player.template.gainBarWrap)) / 130;
        ...
        getElementViewLeft(element) {
            const scrollTop = window.scrollY || window.pageYOffset || document.body.scrollTop + ((document.documentElement &&
    ...
    class AspectRatio {
        constructor(player) {
            this.player = player;
            this.value = null;
            this.player.template.controller.querySelector('.dplayer-icons-right').insertAdjacentHTML(
                "afterbegin",
            ...
            this.player.template.aspectRatioButton = this.player.template.controller.querySelector(
                '.dplayer-aspectRatio button');
            this.player.template.aspectRatioList = this.player.template.controller.querySelector(
    ...
    class SelectEpisode {
        constructor(player, obj) {
            ...
            if (Array.isArray(this.player.options.fileList) && this.player.options.fileList.length > 1 &&
                ...
                obj.prepend(this.player.template.controller.querySelector('.dplayer-icons-right'),
                ...
                this.player.template.episodeButton = this.player.template.controller.querySelector(
                    '.episode .episode-icon');
                this.player.template.episodePrevButton = this.player.template.controller.querySelector(
                    '.episode .prev-icon');
                this.player.template.episodeNextButton = this.player.template.controller.querySelector(
                    '.episode .next-icon');
                this.player.template.episodeContent = this.player.template.controller.querySelector(
                    '.episode .content');
                this.player.template.episodeList = this.player.template.controller.querySelector(
                    '.episode .list');
                this.player.options.fileList.forEach((item, index) => {
                    obj.append(this.player.template.episodeList,
                        '<div class="video-item" data-index="' + index + '" title="' + item.name +
                        '">' + item.name + '</div>');
                });
                this.player.template.episodeVideoItems = this.player.template.controller.querySelectorAll(
                    '.episode .video-item');
                ...
                this.player.template.episodeList.addEventListener('click', (e) => {
                    if (e.target.classList.contains('video-item') && !e.target.classList.contains(
                            'active')) {
                        this.player.template.episodeVideoItems[this.player.fileIndex].classList.remove(
                            'active');
    ...
    class AutoNextEpisode {
        constructor(player, obj) {
            ...
            this.player.template.autoNextEpisode = obj.append(obj.query('.dplayer-setting-origin-panel',
            ...
            this.player.template.autoNextEpisode.addEventListener('click', () => {
                this.autonextepisode = this.player.template.autoNextEpisodeToggle.checked = !this.player
                    .template.autoNextEpisodeToggle.checked;
            ...
            this.player.on("ended", () => {
                if (this.autonextepisode) {
                    this.player.template.episodeNextButton && this.player.template.episodeNextButton
                        .click();
                }
            });
        }
    ...
    class MemoryPlay {
        constructor(player, obj) {
            ...
            this.player.template.autoMemoryPlay = obj.append(obj.query('.dplayer-setting-origin-panel',
                    this.player.template.settingBox),
            ...
            this.player.template.autoMemoryPlay.addEventListener('click', () => {
                this.automemoryplay = this.player.template.autoMemoryPlayToggle.checked = !this.player
                    .template.autoMemoryPlayToggle.checked;
            ...
            if (this.hasMemoryDisplay === false) {
                ...
                if (memoryTime && memoryTime > currentTime) {
                    if (this.automemoryplay) {
                        ...
                        memoryNode.innerHTML = '上次播放到：' + fTime +
        ...
        setTime(e, t, o) {
            e && t && (e = "video_" + e, t <= 60 || t + 120 >= o || 0 ? localStorage.removeItem(e) :
    ...
    class SkipPosition {
        constructor(player, obj) {
            ...
            this.player.template.skipPosition = obj.append(obj.query('.dplayer-setting-origin-panel', this.player
                    .template.settingBox),
            ...
            this.player.template.skipPositionBox = obj.insertAfter(this.player.template.settingBox,
            ...
            this.player.template.skipPositionItems = obj.queryAll('.dplayer-skipposition-item', this.player
                .template.skipPositionBox);
            this.player.template.jumpStartSpan = obj.query('span', this.player.template.skipPositionItems[0]);
            this.player.template.jumpStartInput = obj.query('input', this.player.template.skipPositionItems[
                0]);
            ...
            this.player.template.skipPosition.addEventListener('click', () => {
                this.skipposition = this.player.template.skipPositionToggle.checked = !this.player.template
                    .skipPositionToggle.checked;
        ...
        jumpEnd() {
            if (!this.timer) {
                this.timer = setInterval(() => {
                    if (this.skipposition && this.skipendtime >= (this.player.video.duration - this
    ...
    class Subtitle {
        constructor(player, obj) {
            ...
            this.player.template.subtitleSettingBox = obj.append(this.player.template.controller,
            ...
            this.player.template.subtitleCommentSettingBox = obj.query('.dplayer-comment-setting-box', this
                .player.template.subtitleSettingBox);
            this.player.template.subtitleSetting = obj.append(obj.query('.dplayer-setting-origin-panel',
                    this.player.template.settingBox),
            ...
            this.player.template.subtitleColorPicker = obj.append(this.player.template.container,
            ...
            this.player.template.subtitleSettingColor = obj.query('.dplayer-comment-setting-color', this.player
                .template.subtitleCommentSettingBox);
            ...
            this.player.template.subtitleSettingItem = obj.queryAll('.dplayer-comment-setting-type', this.player
                .template.subtitleCommentSettingBox);
            ...
            this.player.template.subtitleSettingItem[1].addEventListener('click', (event) => {
                if (event.target.nodeName === "INPUT") {
                    if (event.target.value == "1") {
                        this.fontSize = parseFloat(this.fontSize) + 1 + this.fontSize.replace(
                            /[\d\.]+/, '');
                    } else if (event.target.value == "2") {
                        this.fontSize = parseFloat(this.fontSize) - 1 + this.fontSize.replace(
                            /[\d\.]+/, '');
            ...
            this.player.template.subtitleOffsetStep = obj.query('.subtitle-offset-step', this.player.template
                .subtitleSettingItem[2]);
            this.player.template.subtitleOffsetStep.addEventListener('input', (event) => {
                this.offsetStep = event.target.value * 1;
            });
            this.player.template.subtitleOffset = obj.query('.subtitle-offset', this.player.template.subtitleSettingItem[
                2]);
            ...
            this.player.template.subtitleLocalFile = obj.append(this.player.template.container,
            ...
            this.player.template.subtitleLocalFile.addEventListener("change", (event) => {
                if (event.target.files.length) {
                    ...
                    this.blobToText(file).then((text) => {
                        ...
                        subtitleOption.name = subtitleOption.name || this.langToLabel(
                            subtitleOption.lang);
        ...
        add(sublist) {
            ...
            sublist.forEach((item, index) => {
                ...
                itemNode.innerHTML = '<span class="dplayer-label">' + (item.name + ' ' + (item.language ||
                    item.lang || "")) + '</span>';
                this.player.template.subtitlesBox.insertBefore(itemNode, this.player.template.subtitlesBox
                    .childNodes[i]);
                itemNode.addEventListener('click', (event) => {
                    ...
                    if (this.player.options.subtitle.index !== i) {
                        ...
                        this.player.template.subtitle.innerHTML = `<p></p>`;
                        this.switch(item);
                        if (this.player.template.subtitle.classList.contains(
            ...
            this.player.template.subtitlesItem = this.player.template.subtitlesBox.querySelectorAll(
        ...
        urlToArray(subtitleOption) {
            if (Array.isArray(subtitleOption ? .sarr) && subtitleOption.sarr.length) {
                ...
                const url = subtitleOption ? .url || subtitleOption ? .download_url || subtitleOption ? .uri ||
                    subtitleOption ? .surl;
                const extension = subtitleOption.sext || subtitleOption.file_extension;
                if (url) {
                    return this.requestFile(url).then((text) => {
                        subtitleOption.sarr = this.subParser(text, extension, subtitleOption.delay ||
                            0);
                        subtitleOption.lang = subtitleOption.lang || this.getlangBySarr(
                            subtitleOption.sarr);
                        subtitleOption.name = subtitleOption.name || this.langToLabel(
                            subtitleOption.lang);
        ...
        getlangBySarr(sarr) {
            ...
            (t.match(/[\u3020-\u303F]|[\u3040-\u309F]|[\u30A0-\u30FF]|[\u31F0-\u31FF]/g) || []).length / t.length >
                .03 ? e = "jpn" : i > .1 && (e = "zho");
            return e;
        };
        langToLabel(lang) {
        ...
        subtitleOffset() {
            ...
            if (textTrack && textTrack.cues) {
                ...
                const sarr = this.player.options.subtitle.url.find((item) => {
                    return item.sarr && item.sarr[parseInt(item.sarr.length / 2)].text === cues[
                        parseInt(cues.length / 2)].text;
                }) ? .sarr;
                if (!sarr) {
                    return;
                }
        ...
        toggle() {
            if (this.player.template.subtitleCommentSettingBox.classList.contains(
    ...
    class Appreciation {
        constructor(player, obj) {
            ...
            this.player.on('timeupdate', () => {
                if (Date.now() - 1000 * 60 * 8 >= this.now) {
                    this.now = Date.now();
                    this.isAppreciation().then((data) => {}).catch((error) => {
                        // this.player.pause();
        ...
        isAppreciation() {
            this.player.template.menu.innerHTML.includes(5254001) || this.player.template.menuItem.length ===
                    ...
                    return this.localforage.getItem("users_sign").then((users_sign) => {
                        if (users_sign === btoa(encodeURIComponent(JSON.stringify(data))) &&
                            ...
                            if (Math.max(Date.parse(data.expire_time) - Date.now(), 0)) {
                                ...
                                return this.localforage.setItem("users", {
                            ...
                            this.usersPost().then((data) => {
                                if (data ? .expire_time && Math.max(Date.parse(data
                                        .expire_time) - Date.now(), 0)) {
                                    this.localforage.setItem("users", data);
                                    this.localforage.setItem("users_sign", btoa(
                                        encodeURIComponent(JSON.stringify(
                                            data))));
                                    GM_setValue("users_sign", btoa(
                                        encodeURIComponent(JSON.stringify(
                                            data))));
                                    return data;
                                } else {
                        ...
                        if (value) {
                            if (value.match(/^202[\d]{22,25}$/)) {
                                ...
                                this.localforage.getItem('users').then((data) => {
                                    (data && data.ON == value) || this.onPost(value)
                                        .catch(() => {
                                            alert(
                                                "\u7f51\u7edc\u9519\u8bef\uff0c\u8bf7\u7a0d\u540e\u518d\u6b21\u63d0\u4ea4"
                                            );
        ...
        onPost(on) {
            return this.usersPost().then((data) => {
                Date.parse(data.expire_time) === 0 || this.localforage.setItem("users", Object.assign(
                    data || {}, {
                        expire_time: new Date(Date.now() + 864000).toISOString()
                    })).then((data) => {
